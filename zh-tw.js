...
settings.post.ssaoRadius 屏幕空間環境光屏蔽半徑
...
settings.interface.oldBrowser 使用經典服務器瀏覽器
...
settings.interface.kd Show K/D Counter 顯示K/D
...
settings.serverb.header 服務器引擎
settings.serverb.oldBrowser 舊版服務器引擎展示風格
settings.serverb.defaultRegionOnly 僅顯示默認地區服務器
...
settings.viewmodel.weapSwapY 武器切換時的Y軸坐標
settings.viewmodel.weapReloadY 武器切換時的Y軸坐標
settings.viewmodel.yOffADS 武器瞄準Y軸坐標
...
settings.editing.progOffX 進度條水平位置
...
windows.maps.publish 發布遊戲
windows.maps.community 開發者社區
windows.publish.map.header 發布或更新遊戲
...
windows.store.amount 賬戶中有 {0} KR 剩餘
...
custom.map.upload.limit 失敗. 遊戲數據限制為五十萬字符
...
server.config.tickRI 高傳送率(High Tickrate)
...
custom.tryagain 請重試
purchase.kr.success 向您的賬號中添加了{0}KR
limited.purchase.ask 您確定要用 [{0}] 買 {1} KR?
limited.purchase.success 向您的賬戶中添加了{0}KR
pretend.exit 再次點擊 [{0}] 以退出
settings.loaded 設置加載完畢
settings.loaded.none 沒有發現設置
account.login.required 必須登錄
account.banned.hack 此賬號已因為使用外掛被封號
account.banned.appeal 如果您認為您被誤封，請聯繫：
account.delete 您可以請求刪除該賬號；如果此請求在十四天內沒有被撤回，該賬號與其所有相連的數據信息將會被永久抹除
account.delete.request 您確定您要請求刪除該賬號嗎？
account.delete.cancel 您確定您要取消刪除該賬號嗎？
spin.free.available 可以獲取免費KR了！
spin.free.claim 該轉盤僅可在每小時完成一場比賽后使用一次
spin.free.disable 此彈窗可以在通用設置中被取消
mod.reset.0 將會在復活後產生效果
...
stats.sprays 塗鴉次數
windows.host.default.settings 啟用默認設置可以存儲玩家進度和高分
windows.host.default.commands *房主專用指令如/noclip，/god，/smite等已關閉
windows.host.rewards 開啟KR獎勵機制將會強制啟用默認設置並且允許玩家從地圖基金中贏得KR:
windows.store.wheels.sub 試試您的運氣！轉動轉盤以贏得一些新皮膚！
windows.store.currency * 這些價格可以由付款提供商或銀行以匯率計算從美元轉換到您的當地貨幣
windows.store.creator 考慮通過使用您最喜歡的 Krunker 內容創建者的代碼支持他們
windows.chall.customs 挑戰不能在自定義遊戲內進行
twitch.link 您確定您要鏈接您的Twitch賬號嗎？
twitch.unlink 您確定您要取消鏈接您的Twitch賬號嗎？
