module.exports = `
    ...
    settings.quality.post Post-traitement
    ...
    settings.gameplay.slidCont Glissade dépendante des IPS
    ...
    queue.status.queuing En attente
    queue.status.ready-up Pas encore prêt
    queue.status.no-queues Aucune liste d’attente sélectionnée
    queue.status.waiting-for-leader En attente du chef
    queue.status.queued Recherche d’une partie [{0} en attente]
    queue.status.creating-game Partie en cours de création
    queue.status.host Partie en cours de création
    queue.status.join Connexion à la partie en cours
    queue.status.already-queued Déjà en attente
