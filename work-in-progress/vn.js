module.exports = `
menu.btn.ranked Chơi Rank
menu.btn.shop Cửa hàng
menu.btn.hub Krunker Hub
menu.btn.maps Bản đồ/trò chơi khác
menu.btn.mods Mods
menu.btn.settings Cài đặt
menu.btn.customize Tùy chỉnh
menu.btn.invite Mời người chơi
menu.btn.invite.Sao chép
menu.btn.join Tham gia 
menu.btn.host Tạo phòng
menu.btn.chơi Với nhiều người
generic.wait Hãy đợi vài giây...
generic.loading Đang Tải...
generic.none Không
generic.connecting Đang kết nối...
generic.select Chọn
generic.delete Xóa
generic.save Lưu
generic.load Tải lại
generic.search Tìm kiếm
generic.leave Thoát ra
generic.default Mặc Định
generic.submit Nộp
generic.create Tạo
generic.random Ngẫu Nhiên
generic.Ko thành công
generic.success Thành công!
generic.name Tên
generic.level Cấp độ
generic.score Điểm
generic.off Tắt
generic.defeat THẤT BẠI
generic.victory CHIẾN THẮNG
generic.matchover TRẬN ĐẤU KẾT THÚC
generic.matchaband TRẬN ĐẤU BỊ BỎ DỠ
generic.follow Theo dõi
generic.unfollow Bỏ theo dõi
generic.followers Được theo dõi
generic.following Đang theo dõi
generic.games Số lần chơi
generic.wins Thắng
generic.lose Thua
generic.clan Quân đoàn
generic.games.played Số lần chơi
generic.games.won Số lần thắng
generic.games.lost Số lần thua
generic.timep Thòi gian chơi được
generic.kills Giết
generic.deaths Chết
generic.kdr Tỉ lệ Giết/Chết
generic.time Thời gian
generic.logout Đăng Xuẩt
generic.logout.sessions Đăng Xuất tất cả phiên bản
generic.username Nhập tên
generic.password Nhập mật khẩu
generic.register Tạo tài khoản
generic.login Đăng nhập
generic.accuracy Độ chính xác
player.waiting Đang chờ người chơi
player.died Bạn đã chết
player.killed Bị giết bởi
player.killstreak {0} giết được {1} lần liên tục
player.inactive bị Kick vì ko hoạt động
player.disconnect Ko thể kết nối
timer.end Trận đấu tiếp theo trong {0}
leaderboard.sảnh trống 
popup.checkpoint Đã Đánh Dấu!
app.play NHẤP VÀO ĐỂ CHƠI
app.spectate NHẤP VÀO ĐỂ THEO DÕI TRẬN ĐẤU
app.play.mobile NHẬP VÀO ĐỂ CHƠI
app.spectate.mobile NHẤP VÀO ĐỂ THEO DÕI TRẬN ĐẤU
app.play.controller BẤM ĐỂ BẮT ĐẦU
app.spectate.controller BẮT ĐẦU THEO DÕI TRẬN ĐẤU
purchase.error Ôi hỏng
purchase.error.click nhấp vào đây </a> và thử lại!
purchase.error.limit Có thể bạn đã vượt quá giới hạn giao dịch hàng ngày của mình
settings.reset Đặt lại
settings.reset.confirm Bạn có chắc là sẽ đặt lại tất cả cài đặt của bạn ko?
settings.import Nhập mã cài đặt
settings.export Xuất mã cài đặt
settings.none Không tìm thấy cài đặt nào
settings.backup Cài đặt sao lưu
settings.controls.change Cài đặt Phím Tắt
settings.controls.press Bấm một nút bất kì
settings.local.header Localization <span id="requiresRestart"><span style="color: #eb5656">*</span> requires restart</span>
settings.local.region Khu vực mặt định
settings.local.lang Ngôn ngữ
settings.network.header Mạng 
settings.network.sendRate Tỷ lệ đánh dấu cao 
settings.network.netRerouting Định tuyến tự động (Ảnh hưởng đến Ping)
settings.network.lagComp Lag Compensation
settings.network.hitReg Hitreg Test
settings.popup.header Cửa sổ bật lên
settings.popup.freeKRPop Hiển thị cửa sổ bật lên-KR miễn phí
settings.experimental.header Thực nghiệm 
settings.experimental.aimFreezeFix Aim Freeze Fix
settings.experimental.mouseFlickFix Sửa lỗi nhấp chuột
settings.experimental.shaderRendering Instance Rendering
settings.experimental.staticRendering Static Rendering
settings.client.header Client
settings.client.unlimited Khung hình FPS không giới hạn
settings.client.gameCap Game Capture
settings.client.experimental Experimental Features
settings.client.discordRPC Discord RPC
settings.client.resourceSwapper Resource Swapper
settings.client.clearCache Xóa bộ nhớ Cache
settings.client.resetSettings Đặt lại cài đặt của client
settings.client.acceleratedCanvas Accelerated Canvas
settings.client.angleBackend ANGLE Graphics Backend
settings.client.colorProfile Màu hồ sơ
settings.dev.header Developer
settings.dev.forceUpdate Force Update Skins
settings.dev.forceRegen Always Regenerate Player
settings.dev.extraBan Extra Ban Info
settings.chat.header Tin nhắn
settings.chat.chat Hiển thị hộp Chat
settings.chat.chat.all Always
settings.chat.chat.focus While Focused
settings.chat.messages Hiển thị tin nhắn người chơi
settings.chat.unboxings Hiển thị tin nhắn mở hộp (unbox)
settings.chat.chatOp Độ rõ của dòng tin nhắn
settings.chat.chatBGOp Độ trong suốt nền sau của dòng tin nhắn
settings.chat.chatHeight Chiều cao của hộp Chat
settings.chat.chatTimer Thời gian dòng tin nhắn biến mất (giây)
settings.chat.chatTextOutline Hiển thị đường viền văn bản trong tin nhắn
settings.chat.chatFilter Profanity Filter
settings.quality.header Performance
settings.quality.updRate Frame Cap
settings.quality.rendDst Tầm nhìn xa
settings.quality.res Độ phân giải
settings.quality.part Hiển thị những vật rất nhỏ
settings.quality.mapDet Chi tiết bản đồ
settings.quality.noTex Không kết cấu
settings.quality.partdist Tầm nhìn xa những vật nhỏ
settings.quality.renderdist Tầm nhìn xa
settings.quality.reflection chất lượng phản chiếu
settings.quality.antiAlias Antialiasing
settings.quality.shadows Bóng tối
settings.quality.softShad Soft Shadows
settings.quality.highResShad High-Res Shadows
settings.quality.shadowsDynamic bóng năng động
settings.quality.trails Đường đạn 
settings.quality.yourTrails Đường đạn của bạn
settings.quality.mflash Muzzle Flash
settings.quality.flap Hiển thị nắp ống ngắm súng tỉa
settings.quality.lowSpec Low Spec
settings.quality.ambient Bóng râm xung quanh
settings.quality.ambientOld Old Shading (When Possible)
settings.quality.textureAnim Texture Animations
settings.quality.objectAnim Object Animations
settings.quality.screenShake Màn hình rung lắc
settings.quality.weaponShine Chiếu sáng vũ khí
settings.quality.rendGamma Gamma
settings.quality.lighting Anhs sáng
settings.quality.showExplo Hiển thị các vụ nổ
settings.quality.aspectRatio Tỉ lệ khung hình
settings.post.header Post Processing
settings.post.post Post Processing
settings.post.bloom Bloom
settings.post.bloomThreshold Bloom Threshold
settings.post.bloomStrength Bloom Strength
settings.post.bloomRadius Bloom Radius
settings.post.ssao SSAO
settings.post.ssaoRadius SSAO Radius
settings.interface.hitInd Show Hit Indicators
settings.interface.hitColor Hit Indicator Color
settings.interface.dmg Hiển thị Damege
settings.interface.dmgCol Màu Damege
settings.interface.critCol Crit Color
settings.interface.header Interface
settings.interface.scale UI Scale
settings.interface.ui Hiển thị UI
settings.interface.scoreboard Sử dụng bảng điểm cũ
settings.interface.oldBrowser Sử dụng trình duyệt cũ
settings.interface.rarityanim Disable Rarity Animations
settings.interface.showPfp Hiển thị avt hồ sơ
settings.interface.showTradeMsg Hiển thị tin nhắn giao dịch
settings.interface.hideNonTrade Hide Non-Tradable Skins
settings.interface.hp Dynamic Thanh XP
settings.interface.dmgScale Damage Scale
settings.interface.killf Show Kill Feed
settings.interface.ping Hiển thị Ping
settings.interface.net-stats Show Network Stats
settings.interface.fps Hiển thị FPS
settings.interface.deaths Hiển thị số lần chết (kí hiệu bên phải trg trận)
settings.interface.kd Show K/D Counter
settings.interface.kills Hiển thị số lần giết (kí hiệu bên phải trg trận)
settings.interface.streak Hiển thị số giết liên tục (kí hiệu bên phải trg trận)
settings.interface.score Hiển thị điểm (kí hiệu bên phải trg trận)
settings.interface.spectators Hiển thị người đang theo dõi (kí hiệu bên phải trg trận)
settings.interface.speed Hiển thị tốc độ khi bhop
settings.interface.speedOffX Speed X Offset
settings.interface.speedOffY Speed Y Offset
settings.interface.speedOpacity Speed Opacity
settings.interface.speedScale Speed Scale
settings.interface.speedColor Speed Color (Current)
settings.interface.speedColorMax Speed Color (Max)
settings.interface.medals Show Medals
settings.interface.hideNames Hiển thị bảng tên
settings.interface.hideNames.team Chỉ đồng đội
settings.interface.hideNames.enemy Chỉ đối thủ
settings.interface.hideNames.all Tất cả mọi người
settings.interface.nametagStyle Kiểu bảng tên
settings.interface.nametagOpac độ rõ bảng tên
settings.interface.nametagStyle.health chỉ hiển thị máu
settings.interface.nametagStyle.name chỉ hiển thị tên
settings.interface.nametagStyle.nameLvl Chỉ hiển thị tên và cấp độ
settings.interface.nametagStyle.all Hiển thị tất cả
settings.interface.healthNum Hiển thị số máu
settings.interface.healthColT Màu HP đồng đội
settings.interface.healthColE Màu HP địch
settings.interface.xpBarCol Màu thanh XP 
settings.interface.xpBarOpac Độ trong suốt thanh XP
settings.crosshair.header Tâm 
settings.crosshair.type Thể loại
settings.crosshair.type.2 Tùy chỉnh 
settings.crosshair.type.3 Layered
settings.crosshair.type.4 Image
settings.crosshair.type.5 Precision
settings.crosshair.style Thể loại
settings.crosshair.style.0 Dấu cộng
settings.crosshair.style.1 Vòng tròng rỗng
settings.crosshair.style.2 Vòng tròn đầy
settings.crosshair.style.3 Hình vuống rỗng
settings.crosshair.style.4 Hình vuông đầy
settings.crosshair.image Hình ảnh
settings.crosshair.image.paste Hình ảnh Tâm
settings.crosshair.always Luôn hiển thị 
settings.crosshair.color Màu tâm
settings.crosshair.shadow.color Màu viền tâm
settings.crosshair.shadow.thickness Độ dày viền tâm
settings.crosshair.size Kích thước
settings.crosshair.thickness Độ dày
settings.crosshair.gap Khoảng cách tâm 
settings.crosshair.dot dấu chấm
settings.crosshair.opacity.use Sử dụng độ trong suốt tùy chỉnh
settings.crosshair.opacity.custom Độ trong suốt tùy chỉnh
settings.hitmarker.header Hiển thị dấu trúng mục tiêu
settings.hitmarker.color Màu
settings.hitmarker.kcolor Màu giết
settings.hitmarker.thickness Độ dày
settings.hitmarker.opacity Độ trong suốt
settings.hitmarker.leng chiều dài
settings.hitmarker.spacing Spacing
settings.hitmarker.animD Anim Size
settings.hitmarker.animS Anim Speed
settings.hitmarker.hitm Hiển thị
settings.hitmarker.hitmFad Fade Speed
settings.hitmarker.custom Hitmarker Image
settings.hitmarker.custom.paste Hitmarker Image URL
settings.onKillS.style.0 Default
settings.onKillS.style.1 Reduce Resolution
settings.onKillS.style.2 Reduce FOV
settings.onKillS.style.3 Increase FOV
settings.serverb.header Server Browser
settings.serverb.oldBrowser Legacy Browser
settings.serverb.defaultRegionOnly Chỉ khu vực mặc định 
settings.gameprivacy.header Quyền riêng tư trong trò chơi
settings.gameprivacy.streamer Chế độ streamer
settings.gameprivacy.anon Chế độ ẩn danh
settings.gameprivacy.hideGameTitle Ân tiêu đề trò chơi
settings.gameplay.header Gameplay
settings.gameplay.gpSetts Cài đặt Gameplay
settings.gameplay.sensitivityX X Độ nhạy
settings.gameplay.sensitivityY Y Độ nhạy
settings.gameplay.aimsensX Aim X Độ nhạy
settings.gameplay.aimsensY Aim Y Độ nhạy
settings.gameplay.challenge Chế độ thử thách
settings.gameplay.invert Đảo ngược trục-Y
settings.gameplay.onkillS On Kill
settings.gameplay.scrollDir Scroll Direction
settings.controller.header Điều Khiển
settings.controller.sensitivityX X Độ nhạy
settings.controller.disableCons Disable Controllers
settings.controller.sensitivityY Y Độ nhạy
settings.controller.aimsensX Aim X Độ nhạy
settings.controller.aimsensY Aim Y Độ nhạy
settings.controller.deadZoneL Dead Zone (Left)
settings.controller.deadZoneR Dead Zone (Right)
settings.controller.triggerThres Trigger Threshold
settings.controller.vibration Vibration
settings.controller.gradSpeed Gradual Speed
settings.controller.invert Invert Y-Axis
settings.mobile.header Mobile
settings.mobile.sensitivityX X Sensitivity
settings.mobile.sensitivityY Y Sensitivity
settings.mobile.aimsensX Aim X Sensitivity
settings.mobile.aimsensY Aim Y Sensitivity
settings.mobile.vibration Vibration
settings.mobile.gradSpeed Gradual Speed
settings.mobile.invert Invert Y-Axis
settings.audio.header Audio
settings.audio.sound Master Volume
settings.audio.ambient Âm lượng xung quanh
settings.audio.voice Âm lượng tiếng nói
settings.audio.weps Âm lượng vũ khí
settings.audio.player Âm lượng người chơi
settings.audio.skin Cosmetics Volume
settings.audio.ui Âm lượng UI 
settings.audio.asset Âm lượng Asset
settings.audio.dialogue Action Volume
settings.viewmodel.header View Model
settings.viewmodel.vmSetts View Model Settings
settings.viewmodel.fov FOV
settings.viewmodel.fps vũ khí FOV
settings.viewmodel.bobbing Sự rung lắc vũ khí
settings.viewmodel.leaning Độ nghiêng vũ khí
settings.viewmodel.show.primary Hiển thị vũ khí chính
settings.viewmodel.show.secondary Hiển thị vũ khí phụ
settings.viewmodel.roundArms Tay tròn
settings.viewmodel.show.melee Hiển thị Vũ khí cận chiến 
settings.viewmodel.show.ads Ân vũ khí khi ngắm 
settings.viewmodel.toggleAim Toggle Aim
settings.viewmodel.show.hands Hiển thị tay
settings.viewmodel.leftHand Cầm vũ khí bên tay trái
settings.viewmodel.aimAnim Weapon Aim Animation
settings.viewmodel.rot Weapon Rotation
settings.viewmodel.xOff Weapon X Offset
settings.viewmodel.yOff Weapon Y Offset
settings.viewmodel.zOff Weapon Z Offset
settings.viewmodel.weapSwapY Weapon Swap Y
settings.viewmodel.weapReloadY Weapon Reload Y
settings.viewmodel.yOffADS Weapon ADS Y Offset
settings.viewmodel.adsFovMlt ADS FOV Power
settings.editing.header Chỉnh Sửa
settings.editing.health.high HUD Health High
settings.editing.health.low HUD Health Low
settings.editing.speedLines Speed Lines
settings.editing.spdLinesCol Speed Lines Color
settings.editing.score.color Popup Score Color
settings.editing.score.show Show Popup Score
settings.editing.score.scale Popup Score Scale
settings.editing.score.shadow Popup Score Shadow
settings.editing.scoreOffX Popup Score X Offset
settings.editing.scoreOffY Popup Score Y Offset
settings.editing.prog.color Progress Bar Color
settings.editing.prog.opacity Progress Bar Opacity
settings.editing.prog.scale Progress Bar Scale
settings.editing.prog.shadow Progress Bar Shadow
settings.editing.progOffX Progress Bar X Offset
settings.editing.progOffY Progress Bar Y Offset
settings.editing.progRot Progress Bar Rotation
settings.editing.progm.color Charge Bar Color
settings.editing.progm.opacity Charge Bar Opacity
settings.editing.progm.scale Charge Bar Scale
settings.editing.progm.shadow Charge Bar Shadow
settings.editing.progmOffX Charge Bar X Offset
settings.editing.progmOffY Charge Bar Y Offset
settings.editing.progmRot Charge Bar Rotation
settings.editing.saturation Saturation (In-game)
settings.editing.menuSaturation Saturation (UI)
settings.editing.hue Color Hue (In-game)
settings.editing.menuHue Color Hue (UI)
settings.editing.vignette Vignette
settings.editing.feedLimit Killfeed Limit
settings.editing.depth Depth Map
settings.editing.green Green Screen
settings.editing.trailcol Bullet Trail Color
settings.editing.ropecol Grapple Rope Color
settings.editing.tracercol Bullet Tracers
settings.editing.tracer Bullet Tracers
settings.editing.tracer.offset Tracer Offset
settings.shaders.header Shader Inputs
settings.mods.header Customization
settings.mods.load Tải Mod 
settings.mods.logo Cho phép thay đổi Logo
settings.mods.auto Tự động tải Mod
settings.mods.borders Biên giới ống ngắm (súng tỉa)
settings.mods.borderCol Biên giới ống ngắm (súng tỉa)
settings.mods.borderOpac Độ trong suốt biên giới ống ngắm (súng tỉa)
settings.mods.scope Hình ảnh ống ngắm (súng tỉa)
settings.mods.scope.paste Dán URL hình ảnh ống ngắm (súng tỉa)
settings.mods.scope.opacity Độ trong suốt ống ngắm (súng tỉa)
settings.mods.scope.width Chiều rộng hình ảnh ống ngắm
settings.mods.scope.height Chiều cao hình ảnh ống ngắm
settings.mods.dot Hình ảnh tâm
settings.mods.dot.width chiều rộng hình ảnh tâm
settings.mods.dot.height chiều cao hình ảnh tâm
settings.mods.dot.paste tâm URL
settings.mods.endmessage Tin nhắn khi trận đấu kết thúc
settings.mods.profile Hình ảnh hồ sơ (premium)
settings.mods.profile.paste URL Hình ảnh hồ sơ
settings.mods.ammo Hình ảnh ICON đạn
settings.mods.ammo.paste URL hình ảnh icon đạn
settings.mods.kills Hình ảnh Icon giết
settings.mods.kills.paste URL Hình ảnh Icon giết
settings.mods.deaths Hình ảnh Icon chết
settings.mods.deaths.paste URL Hình ảnh Icon chết
settings.mods.streak Hình ảnh Icon giết liên tục
settings.mods.streak.paste URL Hình ảnh Icon giết liên tục
settings.mods.timer hình ảnh icon thời gian
settings.mods.timer.paste  URL Hình ảnh icon thời gian
settings.mods.usedamage Use Damage Overlay
settings.mods.damage Damage Overlay Image
settings.mods.damage.paste Damage Overlay Image URL
settings.mods.overlay Game Overlay Image
settings.mods.overlay.paste Game Overlay Image URL
settings.social.region Region Stats
windows.settings.header Cài đặt game 
windows.mail.header Hộp thư Krunker
windows.servers.header Servers
windows.servers.none Không tìm thấy trò chơi có thể tham gia nào ...
windows.servers.host + tạo phòng
windows.servers.hide Hide Full
windows.servers.priority Region Priority
windows.servers.online {0} Trong trò chơi
windows.servers.social {0} Trên Social
windows.loadout.header Change Loadout
windows.loadout.class class 
windows.loadout.primary Vũ khí chính
windows.loadout.primary.attach kiểu ống ngắm
windows.loadout.primary.reticle Tâm
windows.loadout.primary.scope Ông Ngắm (súng tỉa)
windows.loadout.secondary Vũ khí phụ
windows.loadout.secondary.skin Skin Vũ khí phụ
windows.loadout.melee Vũ khí cận chiến
windows.loadout.hat Mũ
windows.loadout.back Phần thân
windows.loadout.waist Thắt lưng
windows.loadout.shoe Giày
windows.loadout.face Mặt
windows.loadout.spray Icon phun sơn
windows.loadout.color.skin Màu da (pre)
windows.loadout.color.chat Màu tên trong hộp chat
windows.loadout.color.hair Hair Color
windows.loadout.dye Trang phục
windows.loadout.pet Thú
windows.loadout.stat Killcard stat
windows.loadout.flag Cờ/quốc gia
windows.mods.drop thả file mod vào đây hoặc nhấp vào
windows.mods.paste Dán URL mod
windows.mods.load Tải mod
windows.mods.none Không Tìm thấy mod nào cả
windows.mods.header Người quản lí Mod
windows.mods.guide Hướng dẫn mod
windows.mods.list Danh sách mod
windows.mods.load.header Tải mod
windows.mods.publish Tải mod lên 
windows.mods.viewer Viewer
windows.account.header Tài khoản
windows.account.recover Để khôi phục tài khoản hãy liên hệ tới <a>recovery@yendis.ch</a>
windows.class.header Chọn class
windows.controls.header Thay đổi phím
windows.controls.forward Đi tới
windows.controls.backward Đi lui
windows.controls.left sang trái
windows.controls.right sang phải
windows.controls.reload Nạp đoạn
windows.controls.aim Ngắm
windows.controls.shoot Bắn
windows.controls.inspect Quan sát vũ khí/ xem skin
windows.controls.spray Phun sơn
windows.controls.jump Nhảy
windows.controls.crouch Cúi người
windows.controls.prim Chuyển qua vũ khí chính
windows.controls.swap Chuyển qua vũ khí phụ
windows.controls.melee chuyển qua vũ khí cận chiên
windows.controls.equip Đặt Bomb
windows.controls.chat Chat
windows.controls.voice Voice
windows.controls.list Danh sách người chơi
windows.controls.board Bảng điểm 
windows.controls.interact Nhặt vũ khí chính/ sử dụng trigger (donate/skip level)
windows.controls.interactSec Nhặt vũ khí phụ
windows.controls.confirm Xác nhận sau khi nhấn G
windows.controls.drop Thả vũ khí
windows.controls.wepVis Chuyển đổi chế độ hiển thị vũ khí
windows.controls.kickVoteY Vote Kick [Có]
windows.controls.kickVoteN Vote Kick [Ko]
windows.controls.kpdVoteY KPD Vote [Có]
windows.controls.kpdVoteN KPD Vote [Không]
windows.controls.specFree Chuyển đổi Free Cam
windows.controls.specObj Chuyển đổi Cam mục têu
windows.controls.specFirst Chuyển đổi Góc nhìn thứ nhất
windows.controls.specNames Chuyển đổi Forced Names
windows.controls.streak0 Streak 1
windows.controls.streak1 Streak 2
windows.controls.streak2 Streak 3
windows.controls.streak3 Streak 4
windows.controls.streak4 Streak 5
windows.controls.premium0 Noclip
windows.controls.premium1 Godmode
windows.controls.premium2 Kill
windows.controls.premium3 Smite
windows.controls.dev0 Developer Bind 1
windows.controls.dev1 Developer Bind 2
windows.controls.dev2 Developer Bind 3
windows.controls.dev3 Developer Bind 4
windows.controls.reset Reset Position (Race & Parkour)
windows.controls.pretend Ân Game 
windows.host.header Tạo phòng 
windows.host.maps Server Setup
windows.host.cmaps Trò chơi khác 
windows.host.raw Dữ liệu trò chơi (được export từ KrEditor)
windows.host.modes Chọn chế độ 
windows.host.classes Chọn class
windows.host.settings Cài đặt
windows.host.preset Tạo phòng bằng cài đặt trước
windows.host.preset.load Đang tải...
windows.host.preset.name Tên 
windows.host.start Bắt đầu
windows.host.rehost Bắt đầu (Bring Players)
windows.spray.header Chọn icon phun sơn
windows.spray.default Icon phun sơn miễn phí
windows.spray.spin Icon phun sơn
windows.maps.header Game tùy chỉnh
windows.maps.none Không tìm thấy game nào!
windows.maps.name Tên Game
windows.maps.editor Tạo Game
windows.maps.publish Tải game lên 
windows.maps.community Cộng đồng làm game 
windows.publish.map.header Tải game lên hoặc câp nhật game
windows.publish.map.thumbnail Không có nền
windows.publish.map.paste Dán Dữ liệu của game (Được export/copy export từ KrEditor)
windows.publish.map.update Tải game lên/Cập nhật
windows.publish.map.login Đăng nhập để tạo hoặc Upload game
windows.theatre.header Theatre Mode
windows.theatre.record Enable Recording
windows.theatre.key Record Key
windows.clans.header Clans
windows.clans.members Thành viên ({0})
windows.clans.requests Gửi yêu cầu tham gia
windows.clans.requests.none Ko có yêu cầu tham gia 
windows.clans.page [{0}] Clan Page
windows.clans.create Tạo Clan
windows.clans.name Tên Clan 
windows.clans.view Xem Clans
windows.clans.join Tham Clan
windows.clans.login Đăng nhập để tham gia hoặc tạo Clan
windows.store.header Cửa Hàng
windows.store.spin {0} quay
windows.store.agree Bằng cách mua, bạn đồng ý với {0} Điều khoản & Điều kiện
windows.store.purchase Mua KR 
windows.store.voucher Redeem Voucher
windows.store.voucherinfo Redeem Vouchers from certain Streams & Game Events
windows.store.market Khu mua/bán skin
windows.store.market2 Chợ Krunker
windows.store.premium Mua Premium
windows.store.premium2 Krunker Premium
windows.store.buyP Đổi tên và nhiều điều khác ở 
windows.store.buy Mua/Bán skins trên
windows.store.wheels Quay trúng thưởng
windows.store.unlock Mở khóa Skin mới
windows.store.amount {0} KR Trong tài khoản
windows.store.login Đăng nhập để mua và mở khóa Skins
windows.store.purchasing Đang mua...
windows.store.unboxed {0}{1} mở hộp
windows.trade.header Trading
windows.scope.header Chọn ống ngắm
windows.premium.header Premium
windows.skin.header Chọn skin
windows.hat.header Chọn mũ
windows.back.header Chọn Phần thân/lưng
windows.waist.header Chọn Thắt lưng
windows.face.header Chọn mặt
windows.melee.header Chọn vũ khí cận chiến
windows.attach.header Select Attachment
windows.reticle.header Chọn tâm
windows.reticle.unlocked Mở khóa trong lvl {0}
windows.reticle.req Req lvl {0}
windows.dye.header Chọn trang phục
windows.dye.default Trang phục mặc định
windows.dye.spin Trang phục
windows.changelog.header Cập nhật
windows.mods.community.header Community Mods
windows.mods.community.fav Yêu thích 
windows.mods.community.new Mod mới
windows.mods.community.hot 
windows.mods.community.name Tên mod
windows.mods.community.none Không tìm thấy mod nào
windows.publish.mod.header Tải mod lên
windows.publish.mod.thumbnail Không có nền
windows.publish.mod.name Tên mod
windows.publish.mod.paste Mod Url
windows.publish.mod.update Tải lên/cập nhật
windows.publish.mod.login Đăng nhập để tải mod lên hoặc câp nhật mod
windows.secondary.header Chọn vũ khí phụ
windows.secondary.unlocked Đã mở khóa
windows.secondary.customOnly Customs Only
windows.secondary.premiumOnly Premium Only
windows.secondary.req Req lvl {0}
windows.advertise.header Quảng Cáo
windows.support.header Ungr hộ
windows.players.header Danh sách người chơi
windows.players.none Hiện không có người chơi nào ...
windows.join.header Tham gia
windows.join.code Nhập URL/Code
windows.client.header Client
windows.client.windows Windows Installer
windows.client.mac macOS Installer
windows.client.linux Linux Installer
windows.ranked.header Rank
windows.ranked.login Đăng nhập để chơi Rank
windows.ranked.level You must be at least <span color="black">level {0}</span> to play in ranked games! Right now you are at <span color="black">level {1}</span>.
windows.ranked.mode.r1v1 Solo
windows.ranked.mode.r2v2 Duo
windows.ranked.mode.r4v4 Quad
windows.chall.header Thử thách
windows.chall.login Đăng nhập để hoàn thành thử thách!
thumbnail.upload.limit Failed. {0}kb Thumbnail Limit
image.upload.limit Failed. {0}kb Image Limit
thumbnail.upload.error Thumbnail Error
mod.extracting extracting {0}/{1}
mod.disabled mods disabled
mod.error mod error!
mod.loading Đang tải mod...
mod.invalid file mod ko hợp lệ!
matchmaker.full Đã đủ người
matchmaker.full2 Game đã đủ người
matchmaker.updating GAME ĐANG CẬP NHẬT
matchmaker.invalid Game ko có
matchmaker.none Server đã quá sức chứa
adblocker Disable your ad blocker to receive free Krunkies.
ad-failed ko tải được quảng cáo. Vui lòng thử lại. 
streamers.views {0} người xem
streamers.none Ko có stream nào hoạt động
error.extentions If you have any extensions, disable them
error.seek Hãy thử tìm kiếm một trò chơi mới bằng cách nhấp vào 
custom.map.upload.limit Failed. 500k Character Game Data Limit
custom.map.missing Chọn ít nhất một game/bản đồ
custom.mode.missing Chọn ít nhất 1 chế độ
custom.class.missing Chọn ít nhất 1 class
server.config.maxPlayers Số lượng người chơi tối đa
server.config.minPlayers Số lượng người chơi tối thiểu
server.config.lives Lives
server.config.gameTime Phút
server.config.gravMlt Trọng lực
server.config.tickRI High Tickrate
server.config.maxPS Server 40 người chơi
server.config.promServ Promote Server
server.config.jumpMlt Lực nhảy
server.config.airStrf Air Strafing
server.config.deltaMlt Time Scale
server.config.strafeSpd Strafe Speed
server.config.fiRat Fire Rate
server.config.reSpd Tốc độ nạp đạng
server.config.slidTime Slide Time
server.config.fixMov Fixed Movement
server.config.slidSpd Slide Speed
server.config.healthMlt Health Multiplier
server.config.impulseMlt Weapon Impulse
server.config.nameTeam1 Team 1 Name
server.config.nameTeam2 Team 2 Name
server.config.nameTeam3 Team 3 Name
server.config.nameTeam4 Team 4 Name
server.config.nameTeam5 Team 5 Name
server.config.t1Dmg Team 1 Damage
server.config.t2Dmg Team 2 Damage
server.config.t3Dmg Team 3 Damage
server.config.t4Dmg Team 4 Damage
server.config.t5Dmg Team 5 Damage
server.config.selTeam Chọn team
server.config.allowSpect Spectating
server.config.killRewards Kill Rewards
server.config.headshotOnly Chỉ trúng đầu
server.config.noSecondary Disable Secondaries
server.config.noStreaks Disable Streaks
server.config.canSlide Sliding
server.config.wallJP Lực wall jump
server.config.autoJump Tự động nhảy
server.config.bDrop Bullet Drop
server.config.thirdPerson Góc nhìn thứ 3 
server.config.requireCaptcha Require Captcha
server.config.nameTags Ân tên
server.config.kCams Kill Cams
server.config.noReload Ko nạp đạn
server.config.private riêng tư
server.config.krRewards KR Rewards
server.config.forceDefault Dùng cài đặt chính thức game/bản đồ
server.config.warmupTime Warmup Time
server.config.gamRounds Số trận đấu
server.config.objtvTime Objective Time
server.config.forceSpawn Thời gian tự động respawn
server.config.intermTmr Intermission (s)
server.config.scoreLimit giới hạn số điểm
server.config.keepTScore Keep Team Score
server.config.forceC Force Mode Classes
server.config.logTim Log Time (Race, Parkour)
server.config.hitBoxPad Hitbox Padding
server.config.hpRegen Health Regen
server.config.disableB Vô hiệu hóa biên giới 
server.config.throwMel Ném vũ khí cận chiến
server.config.noCosm Disable Cosmetics
server.config.tstCmp Test Mode
server.config.limitClasses Limit Classes
server.config.tmSize Team Size
server.config.fallDmg Fall Damage
server.config.fallDmgThr Fall Damage Thresh
server.config.noDraws No Draws
server.config.bstOfR Best of
social.login Đăng nhập/tạo tài khoản
social.login.failed Đăng nhập thất bại
social.login.buy Đăng nhập để mua
social.login.sell Đăng nhập để bán
social.login.gift Đăng nhập để tặng quà
social.login.proccessing Logging in...
social.since kể từ 
social.user.maps User Games
social.user.mods User Mods
social.maps.none Không tìm thấy game nào...
social.mods.none Không tìm thấy mod nào...
social.market.price.low Giá thấp nhất
social.market.price.list Ra giá bằng
social.market.searching Tìm kiếm
social.market.listed listed {0} ago
social.market.list Ra giá Item
social.market.unlist Gỡ xuống
social.market.unlisting Đang gỡ...
social.market.posting Đang post item...
social.market.req lvl {0} required
social.market.cant Ko thể mua được
social.market.purchase Mua
social.market.fee No List Fee
social.market.failed Thất bại. Vui lòng thử lại
social.market.head Đầu
social.market.back Phần thân
social.market.waist Thắt lưng
social.market.face Mặt
social.market.shoe Giày
social.market.pet Thú 
social.market.melee Vũ khí cận chiến
social.market.spray Phun sơn 
social.market.dye Trang phục
social.market.none Ko tìm được item nào
social.market.login Bạn cần phải đăng nhập.
social.market.grace chuẩn bị trong {0}
social.market.sort lọc / sắp xếp
social.market.filter Tìm kiếm
social.market.sort2 Sắp xếp
social.market.price.high Giá cao nhất
social.trading.submitting Submitting Offer...
social.market.gift.amount Gift Amount in
social.market.fee.gift Ko phí
social.market.gift Tặng KR
social.market.gift.msg Để lại lời nhắn
respawn.auto Tự động respawn trong {0}
custom.host.ranked Không thể tổ chức trận đấu khi đang đấu hạng
funds.insufficient Insufficient Funds
funds.out Có vẻ như bạn đã hết Krunkies
custom.passcode.long Mật khẩu quá dài
custom.passcode.input Nhập mật khẩu của server để tham gia
custom.tryagain Vui lòng thử lại.
purchase.kr.success Đã thêm {0} KR vào tài khoản của bạn
limited.purchase.ask Bạn có chắc là muốn mua [{0}] bằng {1} KR?
limited.purchase.success Đã thêm {0} vào kho đồ của bạn
pretend.exit bấm [{0}] một lần nữa để thoát ra
settings.loaded Cài đặt đã tải
settings.loaded.none Ko tìm thấy cài đặt nào
account.login.required Cần phải đăng nhập 
account.banned.hack Tài khoản của bạn đã bị cấm do sử dụng HACK
account.banned.appeal Nếu bạn muốn khiếu nại lệnh cấm của mình, hãy liên hệ:
account.delete Bạn có thể yêu cầu xóa tài khoản của mình, nếu yêu cầu không được hủy trong vòng 14 ngày, tài khoản và tất cả dữ liệu liên quan sẽ bị xóa vĩnh viễn.
account.delete.request Bạn có chắc chắn muốn yêu cầu xóa tài khoản không?
account.delete.cancel Bạn có chắc chắn muốn hủy xóa tài khoản không?
spin.free.available KR MIỄN PHÍ CÓ SẴN!
spin.free.claim Vòng quay này có sẵn để yêu cầu một lần mỗi giờ sau khi hoàn thành một trận đấu.
spin.free.disable Có thể tắt các Cửa sổ bật lên này trong phần cài đặt 
mod.reset.0 Will có hiệu lực sau khi hồi sinh.
mod.reset.1 Map Objects dont get reset
windows.profile.picture Đổi hình ảnh
stats.assists Assists
stats.nukes Nukes
stats.melee Melee
stats.fistkills Beatdowns
stats.thrownkills Bullseyes
stats.headshots Trúng đầu
stats.wallbangs Xuyên tường
stats.sprays Sprays Placed
windows.host.default.settings Bật Cài đặt mặc định cho phép lưu trữ Tiến trình của người chơi và Điểm cao
windows.host.default.commands *Host Commands are Disabled in this mode: /noclip /god /smite etc
windows.host.rewards Việc bật Phần thưởng KR buộc Máy chủ sử dụng Cài đặt mặc định & cho phép người chơi kiếm KR từ quỹ bản đồ:
windows.store.wheels.sub Hãy thử vận may của bạn và quay để có cơ hội giành được một số skin mới!
windows.store.currency * Giá bằng USD có thể được chuyển đổi sang tiền tệ của bạn theo tỷ giá hối đoái do nhà cung cấp thanh toán hoặc ngân hàng xác định
windows.store.creator Cân nhắc hỗ trợ Người tạo nội dung yêu thích của bạn bằng cách sử dụng Mã người sáng tạo
windows.chall.customs Không thể hoàn thành các thử thách trong trận đấu custom
twitch.link Bạn có chắc chắn muốn liên kết một tài khoản twitch không?
twitch.unlink Bạn có chắc chắn muốn hủy liên kết tài khoản twitch của mình không?
windows.clan.leave Bạn có chắc chắn muốn rời khỏi Clan không?
windows.clan.delete Bạn có chắc chắn muốn xóa Clan của mình không?
windows.clan.kick Bạn có chắc chắn muốn kick {0} ra khỏi clan không?
windows.store.premium.purchase Bạn có chắc chắn muốn mua {0} ngày Premium không?
windows.store.premium.gift Bạn có chắc chắn muốn Tặng '{0}' {1} ngày Premium không?
windows.playerlist.ban Bạn có chắc chắn muốn cấm {0} khỏi trò chơi này không?
windows.playerlist.flag Bạn có chắc muốn Flag {0} không?
windows.playerlist.unflag Bạn có chắc chắn muốn Unflag {0} không?
windows.playerlist.punish Bạn có chắc chắn muốn thực hiện hành động trên {0} không?
item.view.open Mở Model Viewer
item.view.more Thêm thông tin
item.sell.cant Không thể bán mặt hàng.
item.sell.quick Bạn có chắc chắn muốn Bán nhanh {0} không?
social.profile.stats Overall Stats
social.profile.class stats
social.profile.sales Bán
social.profile.sales.market Market Sales
social.profile.skins Skins [{0}]
social.profile.skins.none Không có skin nào được tải lên từ người dùng này
social.profile.games Games [{0}]
social.profile.games.none Không có game nào được tải lên từ người dùng này
social.profile.assets Assets [{0}]
social.profile.assets.none Không có asset nào được tải lên từ người dùng này
social.profile.mods Mods [{0}]
social.profile.mods.none Không có mod nào được tải lên từ người dùng này
social.search kết quả tìm kiếm
social.market.stats.rare Rarest Items
social.market.stats.circulation In Circulation
social.market.stats.units đơn vị Trên toàn thế giới
social.trades.in Gửi đến 
social.trades.in.head Gửi đến  ({0})
social.trades.out Gửi đi
social.trades.out.head Gửi đi ({0})
social.trades.history Đã trao đổi
social.trades.history.head Đã trao đổi ({0})
social.trades.decline Từ chối tất cả
social.trades.message cho phép bật tín nhắn khi trade trong cài đặt
social.trades.warning Bạn không nhận được item nào từ giao dịch này!
social.trades.accept Bạn có chắc chắn muốn đồng ý yêu cầu này không?
stats.airdrops Thính KR
settings.quality.flapAnim Nắp ống ngắm rung động (súng tỉa)
`;
